...
def isValidImportFilename(filenameToImport):
    if not error_on_path_redirection:
        return True
    filtered = re.sub(r"(\/|\\|\.\.+)", '', filenameToImport)
...
def assembleTarget(master, depth=maxFileRecursionDepth):
    ...
    while cur_index < len(master_content_list):
        ...
        if match := regex_import_comp.match(cur_line):
            if importFile := match.groups()[0]:
                # Found import statement
                print(
                    f"{print_prefix}Importing file: {importFile.ljust(maxFilnameChars)}",
                    end="",
                )

                if not isValidImportFilename(importFile):
                    raise f"Invalid filename {importFile}. Do not include path redirections"
...
def main():
    ...
    if (hash_original == hash_new):
        ...
        print("File differs")
        print(f"Writing config to file {target_file}")
